...
router.post('/api/enroll', async (req, res) => {
    try {
      ...
      // Use Mongoose to query the collection for the user's data on the specified date
      const enrollCollection = mongoose.connection.collection(collectionName);
      const existingEnrollment = await enrollCollection.findOne(selectedUserData);

      if (existingEnrollment) {
      // User has already enrolled on this day
        return res.status(400).json({ error: 'Duplicate enrollment. User has already enrolled on this day.' });
      }
